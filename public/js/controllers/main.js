var cont = angular.module('todoController', ['ui.bootstrap', 'tagged.directives.infiniteScroll', 'ngAnimate', 'angularGrid', 'ng', 'ngCookies']);


// inject the Todo service factory into our controller
cont.controller('MasterController', ['$scope', '$http', 'Todos', '$uibModal', 'metro', 'metrolatlon', 'imageService', 'angularGridInstance', '$mdSidenav', '$mdDialog', '$cookies', '$location', '$log', function($scope, $http, Todos, $uibModal, metro, metrolatlon, imageService, angularGridInstance, $mdSidenav, $mdDialog, $cookies, $location, $log) {
	...
	$scope.getLatLon = function(xmetro) {
			...
			Todos.searchEvents(xmetro, $scope.chosenDay,  $scope.page)
				.success(function(data) {
					  ...
					  $scope.chosenLonlat = data.latlng;
						$scope.eventList = data.results;
						eventList = $scope.eventList;
						//$scope.loading = false;
				
							Todos.getCities($scope.chosenDay, $scope.chosenLonlat)
								.success(function(data) {
		...
		// Using callback 
		geocoder.geocode(xmetro, function(err, res) {
  		console.log(res);
			
			$scope.chosenLonlat = res.latitude+','+res.longitude;
...
**/
	...
	Todos.getCities($scope.chosenDay, $scope.chosenLonlat)
		.success(function(data) {
			//inject current location option if geo allowed
			if (navigator.geolocation) {
				data.unshift({_id: "Near Me"});
				//cur location
			}
			data.unshift({_id: "Change Metro"});
			$scope.cityList = data;
			$scope.loading = false;
		});
}
	...
	//populates list of metros pop-up
	Todos.getAllMetros()
		.success(function(data) {
			$scope.metroList = data;
		});
