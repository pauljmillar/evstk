angular.module('todoService', ['angularGrid'])

	// super simple service
	// each function returns a promise object
	.factory('Todos', ['$http',function($http) {
		return {
			searchEvents : function(city, chosenDay, page) {
				return $http.get('/api/events/search/' + city + '/'+ chosenDay + '/' + page);
			},
			getCities : function(chosenDay, latlon) {
				return $http.get('/api/cities/' + chosenDay + '/' + latlon);
			},
			...
			getAllMetros : function() {
				return $http.get('/api/metros');
			},
			getNearestMetro : function(latlon) {
				return $http.get('/api/metro/' + latlon);
			},
			...
			updateAbuseFlagCount : function(id) {
				return $http.put('/api/events/abuse/' + id).then(function (status){
                                  return status.data;
                                });
			},
